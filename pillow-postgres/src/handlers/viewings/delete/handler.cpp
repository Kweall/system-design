...
ViewingsDeleteHandler::ViewingsDeleteHandler(
...
std::string ViewingsDeleteHandler::ExtractToken(const userver::server::http::HttpRequest& request) const {
...
bool ViewingsDeleteHandler::ValidateToken(const std::string& token, std::string& username, int64_t& user_id) const {
...
userver::formats::json::Value ViewingsDeleteHandler::HandleRequestJsonThrow(
    ...
    if (owner_id != user_id) {
        throw userver::server::handlers::ClientError(
            userver::server::handlers::ExternalBody{"You don't have permission to delete this viewing"});
    }
    ...
    if (result.IsEmpty()) {
        throw userver::server::handlers::ClientError(
            userver::server::handlers::ExternalBody{"Failed to delete viewing"});
    }
    
    userver::formats::json::ValueBuilder response;
    response["message"] = "Viewing cancelled successfully";
